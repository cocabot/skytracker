...
- **FAI 円筒タスク**: TAKEOFF / SSS / TURNPOINT / ESS / GOAL、EXIT スタート
- **最適化ルート**: 円筒をかすめる最短経路（XCTrack / XCSoar と同系統）
- **レース HUD**: 次パイロン距離・方位、残距離、到着余高、STF、融合風
- **マッククリーディ速度**: 予想上昇率と風を入れたスピード・トゥ・フライ
- **ファイナルグライド**: 風三角で必要高度・到着高度・カニ角を計算
- **CUP / XCTrack `.xctsk` 読み込み**: サンプルタスク（東京デモ・朝霧クラシック）付き

### 気象・サーマル（レースを有利にするレイヤー）

- **Open-Meteo** の観測相当 current ＋ 予報（JMA MSM / GFS / ECMWF / ICON を切替可能）
- **高度別の風**（接地 10 m + 925–500 hPa の飛行帯）と地図上の風ベクトル
- **GPS 旋回風**とモデル風の融合（XCTrack のドリフト推定に相当）
- **サーマル予測**: CAPE・LI・日射・混合層高度・地形（尾根・南斜面）から強度と位置を推定
- **風下ドリフト**とクラウドストリート、実測コア（旋回アシスタント）

ヘッダーの「レース」からタスクを読み込み、地図上の予測サーマルを見ながらパイロンを回ります。
